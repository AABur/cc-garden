...
class TestParser(unittest.TestCase):
    ...
    def test_dedup_by_message_and_request_id(self):
        ...
        sess, _causal_events, _stats = jp.build_session_from_records("s1", lines)
    ...
    def test_session_file_time_window_filters_old_records(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            sess, _causal_events, _stats, bad = jp.parse_session_file(p, since=since)
    ...
    def test_session_file_counts_bad_lines(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            p = Path(d) / "s1.jsonl"
            p.write_text(good + "\n{not valid json\n", encoding="utf-8")
            sess, _causal_events, _stats, bad = jp.parse_session_file(p, since=None)
